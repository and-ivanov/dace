...
class WCRExtraction(pattern_matching.Transformation):
    ...
    @staticmethod
    def expressions():
        return [sdutil.node_path_graph(WCRExtraction._tasklet, WCRExtraction._map_exit, WCRExtraction._output_node)]
    ...
    def apply(self, sdfg: SDFG):
        ...
        output_node = sdfg_state.nodes()[self.subgraph[WCRExtraction._output_node]]

        edge_pairs = WCRExtraction.get_edges(sdfg_state, tasklet, map_exit, output_node)
        ...
        for int_edge, ext_edge in edge_pairs:
            ...
            me, mx = sdfg_state.add_map('extracted_wcr', {str(k): v for k, v in zip(map.params, map_range_str)})
            id_array_name, id_array = sdfg.add_transient('identity_array', [1], dtype)
            ...
            transient_name, transient_array = sdfg.add_transient('extracted_wcr_tmp', map_range.size(), dtype)
            ...
            sdfg_state.add_edge(tasklet, int_src_conn, map_exit, int_dst_conn, dace.Memlet(transient_indexing))
            sdfg_state.add_edge(map_exit, ext_src_conn, transient_access, None, dace.Memlet(transient_name))
            sdfg_state.add_edge(transient_access, None, me, 'IN_ID_ENTRY', dace.Memlet(transient_name))
            sdfg_state.add_edge(me, 'OUT_ID_ENTRY', id_access, None, dace.Memlet(transient_indexing))
            sdfg_state.add_edge(id_access, None, mx, 'IN_ID_EXIT', int_memlet)
            sdfg_state.add_edge(mx, 'OUT_ID_EXIT', output_node, None, ext_memlet)